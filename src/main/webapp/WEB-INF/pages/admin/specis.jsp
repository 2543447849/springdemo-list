...
<div class="container">
    ...
    <c:if test="${empty speciList}">
        <div class="alert alert-warning" role="alert">
            <span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span>specis表为空，请<a href="/admin/speci/add" type="button" class="btn btn-primary btn-sm">添加</a>
        </div>
    </c:if>
    ...
    <c:if test="${!empty speciList}">
        <table class="table table-bordered table-striped">
            ...
            <c:forEach items="${speciList}" var="speci">
                <tr>
                    <td><c:if test="${speci.id} == '0'">雄性</c:if></td>
                    <td>${speci.name}</td>
